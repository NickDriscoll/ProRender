	...
	VkPhysicalDeviceDescriptorIndexingFeatures descriptor_indexing_features = {};
	{
		...
		for (uint32_t j = 0; j < 3; j++) {
			for (uint32_t i = 0; i < physical_device_count; i++) {
				...
				vkGetPhysicalDeviceQueueFamilyProperties2(phys_device, &queue_family_count, nullptr);
				...
				vkGetPhysicalDeviceQueueFamilyProperties2(phys_device, &queue_family_count, queue_properties.data());
...
void VulkanGraphicsDevice::tick_image_uploads(VkCommandBuffer render_cb, VkDescriptorSet descriptor_set, uint32_t binding) {
	...
	for (auto batch_it = _image_upload_batches.begin(); batch_it != _image_upload_batches.end(); ++batch_it) {
		...
		for (auto image_it = _pending_images.begin(); image_it != _pending_images.end(); ++image_it) {
			VulkanPendingImage& pending_image = *image_it;
			...
			if (pending_image.batch_id == batch.id) {
				...
				//Descriptor update data
				{
					...
					pending_images_to_delete.push_back(image_it.slot_index());
...
uint32_t DescriptorSetSpec::push_binding(VkDescriptorType type, uint32_t count, VkShaderStageFlags flags) {
	...
	return static_cast<uint32_t>(this->bindings.size() - 1);
}
...
uint32_t DescriptorSetSpec::push_immutable_sampler(VulkanGraphicsDevice& vgd, VkSamplerCreateInfo& info) {
	...
	return static_cast<uint32_t>(immutable_samplers.size() - 1);
}
